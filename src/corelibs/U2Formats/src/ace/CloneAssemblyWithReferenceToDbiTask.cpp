...
void CloneAssemblyWithReferenceToDbiTask::run() {
    ...
    SAFE_POINT_EXT(NULL != clonedObject, tr("Can't get the cloned object"), );
    QScopedPointer<AssemblyObject> clonedAssemblyObject(qobject_cast<AssemblyObject *>(clonedObject.data()));
    SAFE_POINT_EXT(NULL != clonedAssemblyObject, tr("Unexpected result object: expect AssemblyObject, got %1 object").arg(clonedObject->getGObjectType()), );
    clonedObject.take();
    ...
    SAFE_POINT_EXT(NULL != clonedSequenceObject, tr("Unexpected result object: expect U2SequenceObject, got %1 object").arg(clonedObject->getGObjectType()), );
    clonedObject.take();
