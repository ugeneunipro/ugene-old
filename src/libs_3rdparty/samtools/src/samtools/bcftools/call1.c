...
static int test16_core(int anno[16], anno16_t *a)
{
	...
	a->mq = (int)(sqrt((anno[9] + anno[11]) / (float)a->depth) + .499);
...
static char **read_samples(const char *fn, int *_n)
{
	...
	*_n = 0;
	...
	while (ks_getuntil(ks, 0, &s, &dret) >= 0) {
		...
		if (max == n) {
			max = max? max<<1 : 4;
			sam = (char **)realloc(sam, sizeof(void*)*max);
		}
		l = s.l;
		sam[n] = (char *)malloc(s.l + 2);
		strcpy(sam[n], s.s);
...
int bcfview(int argc, char *argv[], UGENE_bcf_view_settings* settings, const char* vcfOut)
{
    ...
    if(settings->b_keepalt == 1) vc.flag |= VC_KEEPALT;
    if(settings->b_fix_pl == 1) vc.flag |= VC_FIX_PL;
    if(settings->b_no_geno == 1) vc.flag |= VC_NO_GENO;
    if(settings->b_acgt_only == 1) vc.flag |= VC_ACGT_ONLY;
    if(settings->bcf_bed && strlen(settings->bcf_bed) != 0) vc.bed = bed_read(settings->bcf_bed);
    if(settings->b_qcall == 1) vc.flag |= VC_QCALL;
    if(settings->samples && strlen(settings->samples) != 0) vc.subsam = read_samples(settings->samples, &vc.n_sub);
    vc.min_smpl_frac = settings->min_smpl_frac;
    if(settings->b_call_gt == 1) vc.flag |= VC_CALL_GT | VC_CALL;
    vc.indel_frac = settings->indel_frac;
    vc.pref = settings->pref;

    if(settings->ptype && strlen(settings->ptype) != 0){
        if (strcmp(settings->ptype, "full") == 0) vc.prior_type = MC_PTYPE_FULL;
        else if (strcmp(settings->ptype, "cond2") == 0) vc.prior_type = MC_PTYPE_COND2;
        else if (strcmp(settings->ptype, "flat") == 0) vc.prior_type = MC_PTYPE_FLAT;
        else vc.prior_file = strdup(settings->ptype);
    }

    vc.theta = settings->theta;
    if(settings->ccall && strlen(settings->ccall) != 0){
        if (strcmp(settings->ccall, "trioauto") == 0) vc.trio_aux = bcf_trio_prep(0, 0);
        else if (strcmp(settings->ccall, "trioxd") == 0) vc.trio_aux = bcf_trio_prep(1, 0);
        else if (strcmp(settings->ccall, "trioxs") == 0) vc.trio_aux = bcf_trio_prep(1, 1);
        else if (strcmp(settings->ccall, "pair") == 0) vc.flag |= VC_PAIRCALL;
        else {
    ...
    vc.n1 = settings->n1;
    vc.n_perm = settings->n_perm;
    vc.min_perm_p = settings->min_perm_p;
	...
	if (vc.n_perm > 0) {
		seeds = (int *)malloc(vc.n_perm * sizeof(int));
		srand48(time(0));
		for (c = 0; c < vc.n_perm; ++c) seeds[c] = lrand48();
	}
	b = (bcf1_t *)calloc(1, sizeof(bcf1_t));
	blast = (bcf1_t *)calloc(1, sizeof(bcf1_t));
	strcpy(moder, "r");
    ...
    bout = vcf_open(vcfOut, modew);
	if (!(vc.flag & VC_QCALL)) {
		if (vc.n_sub) {
			vc.sublist = (int*)calloc(vc.n_sub, sizeof(int));
