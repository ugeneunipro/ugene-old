...
// 4. Check names of alignment files and names of found subsequences
    ...
    for(int i = 2; i > 0; i--) {
        const QString expectedFileName = "SPatte" + QString::number(i) + "_test]" + alignmentFileExtension;
        ...
        switch(i) {
        case 1:
            CHECK_SET_ERR(sequencesNameList[0] == "S115_test]" && sequencesNameList[1] == "Patte316",
                seqNameMismatchErrorMessage + expectedFileName);
            break;

        case 2:
            CHECK_SET_ERR(sequencesNameList[0] == "S3342_test]" && sequencesNameList[1] == "Patte413",
                seqNameMismatchErrorMessage + expectedFileName);
            break;
