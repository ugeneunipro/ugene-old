...
void DisplayLists::drawAtomsBonds(const Color4f &viewAtomColor, float renderDetailLevel, const Molecule3DModel &model, const BioStruct3DColorScheme* colorScheme)
{
...
void DisplayLists::createBigDisplayList(float renderDetailLevel, const BioStruct3D& bioStruct, const QList<int> &modelIndexList, const BioStruct3DColorScheme* colorScheme)
{
    log.trace(QString("Disp Atom: renderDetailLevel %1").arg(renderDetailLevel));

    glDeleteLists(BIG_DL_INDEX, 1);
    // Create the id for the list
    bigDL = glGenLists(BIG_DL_INDEX);

    QList<Color4f> colors;

    glNewList(bigDL,GL_COMPILE);
