...
static void gl2psPrintGzipFooter()
{
  ...
  if(gl2ps->options & GL2PS_COMPRESS){
    ...
    if(Z_OK != gl2psDeflate()){
      ...
      uLong crc = crc32(0L, gl2ps->compress->start, gl2ps->compress->srcLen);
      for(n = 0; n < 4; ++n){
        tmp[n] = (char)(crc & 0xff);
        crc >>= 8;
      }
      uLong len = gl2ps->compress->srcLen;
      for(n = 4; n < 8; ++n){
        tmp[n] = (char)(len & 0xff);
        len >>= 8;
      }
...
static GLint gl2psFindRoot(GL2PSlist *primitives, GL2PSprimitive **root)
{
  ...
  *root = *(GL2PSprimitive**)gl2psListPointer(primitives, 0);
  ...
  if(gl2ps->options & GL2PS_BEST_ROOT){
    GLint i, best = 1000000, index = 0, maxp;
                                              ...
                                              GLfloat xmin, GLfloat ymin)
{
  ...
  if(GL2PS_ZERO(dx*dy)){
    ...
    GLfloat diff = (vertex->xyz[0] - xmin) / dx;
    if(diff > 1)
      diff = 1.0F;
    else if(diff < 0)
      diff = 0.0F;
...
static void gl2psPrintSVGPixmap(GLfloat x, GLfloat y, GL2PSimage *pixmap)
{
  ...
  gl2psListDelete(png);
#else
  Q_UNUSED(x); Q_UNUSED(y); Q_UNUSED(pixmap);
...
static void gl2psPrintPGFDash(GLushort pattern, GLint factor)
{
  ...
  if(!pattern || !factor){
    ...
    int i = 0;
    for(; i < n; i++) fprintf(gl2ps->stream, "{%dpt}", array[i]);
