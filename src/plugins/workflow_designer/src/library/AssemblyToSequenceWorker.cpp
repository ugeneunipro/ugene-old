...
: Task(tr("Fetch sequences from assembly"), TaskFlag_None),
...
void AssemblyToSequencesWorkerFactory::init() {
    ...
    Descriptor protoDesc(AssemblyToSequencesWorkerFactory::ACTOR_ID,
        AssemblyToSequencesWorker::tr("Fetch sequences from assembly"),
        AssemblyToSequencesWorker::tr("Fetches sequences from assembly"));
...
QString AssemblyToSequencesPrompter::composeRichDoc() {
    ...
    QString res = tr("Fetches sequences from %1 assemblies and puts them to the output").arg(producerName).arg(producerName);
    return res;
}
