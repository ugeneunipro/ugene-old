...
//	if(template TYPE IS Ebwt<String<Dna, Packed<>>)
	return false;
}
...
inline static int countInU64(int c, uint64_t dw) {
	uint64_t dwA  = dw &  U64(0xAAAAAAAAAAAAAAAA);
	...
	uint32_t seed = 0)
{
	...
	for(unsigned int i = 0; i < l.size(); i++) {
		if(ctx->isCanceled()) throw BowtieException("Canceled");
		ctx->ti.progress = (ctx->build.secondEbwtRound ? 50.0:0.0) + (ctx->build.doubleEbwt ? 5.0 : 10.0) * (float)i / (float)l.size();
                             ...
                             ostream& out2)
{
	...
	auto_ptr<uint32_t> ftab_helper(NULL);
	...
	uint8_t absorbCnt = 0;
	uint8_t *absorbFtab;
	auto_ptr<uint8_t> absorbFtab_helper(NULL);
	try {
		VMSG_NL("Allocating ftab, absorbFtab");
		ftab = new uint32_t[ftabLen];
		ftab_helper = auto_ptr<uint32_t>(ftab);
		memset(ftab, 0, 4 * ftabLen);
		absorbFtab = new uint8_t[ftabLen];
		absorbFtab_helper = auto_ptr<uint8_t>(absorbFtab);
...
#ifdef SIXTY4_FORMAT
	auto_ptr<uint64_t> ebwtSide_helper(NULL);
	uint64_t *ebwtSide = NULL;
#else
	auto_ptr<uint8_t> ebwtSide_helper(NULL);
	uint8_t *ebwtSide = NULL;
#endif
	try {
#ifdef SIXTY4_FORMAT
		ebwtSide = new uint64_t[sideSz >> 3];
		ebwtSide_helper = auto_ptr<uint64_t>(ebwtSide);
#else
		ebwtSide = new uint8_t[sideSz];
		ebwtSide_helper = auto_ptr<uint8_t>(ebwtSide);
#endif
	} catch(bad_alloc &e) {
	...
	ctx->ti.progress = (ctx->build.secondEbwtRound ? 50.0:0.0) + ( ctx->build.doubleEbwt ? 50 : 100 );
	assert(ftab != NULL);
...
//	}
	...
	auto_ptr<uint32_t> eftab_helper(NULL);
	uint32_t *eftab = NULL;
	try {
		eftab = new uint32_t[eftabLen];
		eftab_helper = auto_ptr<uint32_t>(eftab);
