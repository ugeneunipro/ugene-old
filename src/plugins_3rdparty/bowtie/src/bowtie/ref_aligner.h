...
protected:
	                ...
	                bool seedOnLeft = false) const
	{
		...
		const uint32_t halfway = begin + (lim >> 1);
		uint64_t anchor = U64(0);
		uint64_t buffw = U64(0);  // rotating ref sequence buffer
		// OR the 'diff' buffer with this so that we can always count
		// 'N's as mismatches
		uint64_t diffMask = U64(0);
		// Set up a mask that we'll apply to the two bufs every round
		// to discard bits that were rotated out of the anchor area
		uint64_t clearMask = U64(0xffffffffffffffff);
		bool useMask = false;
		...
		for(uint32_t i = 1; i <= lim + 1; i++) {
			...
			if((diff & U64(0xffffffff00000000)) &&
			   (diff & U64(0x00000000ffffffff))) continue;
			uint32_t ri  = hi ? riLo  : riHi;
			uint32_t rir = hi ? rirLo : rirHi;
			// Could use pop count
			...
			if(diffs > 1) {
				...
				mmpos = 31;
				if((diff & U64(0xffffffff)) == 0) { diff >>= U64(32); mmpos -= 16; }
				assert_neq(0, diff);
				if((diff & U64(0xffff)) == 0)     { diff >>= U64(16); mmpos -=  8; }
				assert_neq(0, diff);
				if((diff & U64(0xff)) == 0)       { diff >>= U64(8);  mmpos -=  4; }
				assert_neq(0, diff);
				if((diff & U64(0xf)) == 0)        { diff >>= U64(4);  mmpos -=  2; }
				assert_neq(0, diff);
				if((diff & U64(0x3)) == 0)        { mmpos--; }
				assert_neq(0, diff);
				assert_geq(mmpos, 0);
				assert_lt(mmpos, 32);
				mmpos -= anchorCushion;
...
protected:
					...
					bool seedOnLeft = false) const
	{
		...
		for(i = 1; i <= lim + 1; i++) {
			...
			if((diff & U64(0xfffff00000000000)) &&
			   (diff & U64(0x00000ffffff00000)) &&
			   (diff & U64(0x00000000000fffff))) continue;
			uint32_t ri  = hi ? riLo  : riHi;
			uint32_t rir = hi ? rirLo : rirHi;
			// Could use pop count
			uint8_t *diff8 = reinterpret_cast<uint8_t*>(&diff);
			...
			if(diffs > 2) {
				...
				uint64_t diff2 = diff;
				mmpos1 = 31;
				if((diff & U64(0xffffffff)) == 0) { diff >>= U64(32); mmpos1 -= 16; }
				assert_neq(0, diff);
				if((diff & U64(0xffff)) == 0)     { diff >>= U64(16); mmpos1 -=  8; }
				assert_neq(0, diff);
				if((diff & U64(0xff)) == 0)       { diff >>= U64(8);  mmpos1 -=  4; }
				assert_neq(0, diff);
				if((diff & U64(0xf)) == 0)        { diff >>= U64(4);  mmpos1 -=  2; }
				assert_neq(0, diff);
				if((diff & U64(0x3)) == 0)        { mmpos1--; }
				assert_neq(0, diff);
				assert_geq(mmpos1, 0);
				assert_lt(mmpos1, 32);
				...
				if(diffs > 1) {
					...
					diff2 &= ~(U64(0xc000000000000000) >> (uint64_t)((mmpos1+anchorCushion) << 1));
					assert_neq(diff2, origDiff2);
					mmpos2 = 31;
					if((diff2 & U64(0xffffffff)) == 0) { diff2 >>= U64(32); mmpos2 -= 16; }
					assert_neq(0, diff2);
					if((diff2 & U64(0xffff)) == 0)     { diff2 >>= U64(16); mmpos2 -=  8; }
					assert_neq(0, diff2);
					if((diff2 & U64(0xff)) == 0)       { diff2 >>= U64(8);  mmpos2 -=  4; }
					assert_neq(0, diff2);
					if((diff2 & U64(0xf)) == 0)        { diff2 >>= U64(4);  mmpos2 -=  2; }
					assert_neq(0, diff2);
					if((diff2 & U64(0x3)) == 0)        { mmpos2--; }
					assert_neq(0, diff2);
					assert_geq(mmpos2, 0);
					assert_lt(mmpos2, 32);
...
protected:
					...
					bool seedOnLeft = false) const
	{
		...
		for(uint32_t i = 1; i <= lim + 1; i++) {
			...
			if((diff & U64(0xffff000000000000)) &&
			   (diff & U64(0x0000ffff00000000)) &&
			   (diff & U64(0x00000000ffff0000)) &&
			   (diff & U64(0x000000000000ffff))) continue;
			uint32_t ri  = hi ? riLo  : riHi;
			uint32_t rir = hi ? rirLo : rirHi;
			// Could use pop count
			uint8_t *diff8 = reinterpret_cast<uint8_t*>(&diff);
			...
			if(diffs > 3) {
				...
				if(diffs > 1) {
					...
					if(diffs > 2) {
						// Figure out the second mismatched position
						diff3 &= ~(U64(0xc000000000000000) >> (uint64_t)((mmpos2orig + anchorCushion) << 1));
						mmpos3 = 31;
						if((diff3 & U64(0xffffffff)) == 0) { diff3 >>= U64(32); mmpos3 -= 16; }
						assert_neq(0, diff3);
						if((diff3 & U64(0xffff)) == 0)     { diff3 >>= U64(16); mmpos3 -=  8; }
						assert_neq(0, diff3);
						if((diff3 & U64(0xff)) == 0)       { diff3 >>= U64(8);  mmpos3 -=  4; }
						assert_neq(0, diff3);
						if((diff3 & U64(0xf)) == 0)        { diff3 >>= U64(4);  mmpos3 -=  2; }
						assert_neq(0, diff3);
						if((diff3 & U64(0x3)) == 0)        { mmpos3--; }
						assert_neq(0, diff3);
...
protected:
					...
					bool seedOnLeft = false) const
	{
		...
		for(uint32_t i = 1; i <= lim + 1; i++) {
			...
			if((diff & U64(0xf00f00f00f00f00f)) &&
			   (diff & U64(0x0f00f00f00f00f00)) &&
			   (diff & U64(0x00f00f00f00f00f0))) continue;
			if((diff & U64(0xc30c30c30c30c30c)) &&
			   (diff & U64(0x30c30c30c30c30c3)) &&
			   (diff & U64(0x0c30c30c30c30c30))) continue;
			uint32_t ri  = hi ? riLo  : riHi;
			uint32_t rir = hi ? rirLo : rirHi;
			// Could use pop count
			uint8_t *diff8 = reinterpret_cast<uint8_t*>(&diff);
			...
			if(diffs > 2) {
				...
				if(diffs > 1) {
					// Figure out the second mismatched position
					ASSERT_ONLY(uint64_t origDiff2 = diff2);
					diff2 &= ~(U64(0xc000000000000000) >> (uint64_t)((savedMmpos1) << 1));
					assert_neq(diff2, origDiff2);
					mmpos2 = 31;
					if((diff2 & U64(0xffffffff)) == 0) { diff2 >>= U64(32); mmpos2 -= 16; }
					assert_neq(0, diff2);
...
protected:
					...
					bool seedOnLeft = false) const
	{
		...
		for(uint32_t i = 1; i <= lim + 1; i++) {
			...
			if((diff & U64(0xf000f000f000f000)) &&
			   (diff & U64(0x0f000f000f000f00)) &&
			   (diff & U64(0x00f000f000f000f0)) &&
			   (diff & U64(0x000f000f000f000f))) continue;
			if((diff & U64(0xc003c003c003c003)) &&
			   (diff & U64(0x3c003c003c003c00)) &&
			   (diff & U64(0x03c003c003c003c0)) &&
			   (diff & U64(0x003c003c003c003c))) continue;
			uint32_t ri  = hi ? riLo  : riHi;
			uint32_t rir = hi ? rirLo : rirHi;
			// Could use pop count
			uint8_t *diff8 = reinterpret_cast<uint8_t*>(&diff);
			...
			if(diffs > 3) {
				...
				if(diffs > 1) {
					...
					if(diffs > 2) {
						...
						diff3 &= ~(U64(0xc000000000000000) >> (uint64_t)((savedMmpos2) << 1));
