	...
	while(--limit >= 0) {
		...
		// message.  (This step can take a long time.)
		{
			...
			for(uint32_t iten = 0, ten = 0; iten < len; iten += lenDiv10, ten++) {
				...
				if(ctx->isCanceled()) {
					clear(_sampleSuffs);
					throw BowtieException("Canceled");
				}
	...
	if(length(_sampleSuffs) == 0) {
		...
		try {
			...
			if(this->_passMemExc) {
				...
				ostringstream err;
				err << "Could not allocate a suffix-array block of " << ((this->bucketSz()+1) * 4) << " bytes" << endl;
				err << "Please try using a larger number of blocks by specifying a smaller --bmax or" << endl
					<< "a larger --bmaxdivn" << endl;
		...
		try {
			...
			if(this->_passMemExc) {
				...
				ostringstream err;
				err << "Could not allocate a z-array of " << (_dcV * 4) << " bytes" << endl;
				err << "Please try using a larger number of blocks by specifying a smaller --bmax or" << endl
					<< "a larger --bmaxdivn" << endl;
				throw BowtieException(err.str());
			}
		...
		assert_eq(0, length(bucket));
		{
			...
			for(uint32_t iten = 0, ten = 0; iten < len; iten += lenDiv10, ten++) {
				uint32_t itenNext = iten + lenDiv10;
				float localProgress = (_cur + ten/10.0) / ((float)length(_sampleSuffs)+1) ;
				ctx->ti.progress = (ctx->build.secondEbwtRound ? 50.0:0.0) + (ctx->build.doubleEbwt ? 0.5 : 1.0) * (40.0 + 60.0 * localProgress);
				if(ctx->isCanceled()) {
					throw BowtieException("Canceled");
				}
				if(ten > 0) VMSG_NL("  " << (ten * 10) << "%");
				for(uint32_t i = iten; i < itenNext && i < len; i++) {
					...
					try {
						...
						if(this->_passMemExc) {
							...
							ostringstream err;
							err << "Could not append element to block of " << ((length(bucket)) * 4) << " bytes" << endl;
							err << "Please try using a larger number of blocks by specifying a smaller --bmax or" << endl
								<< "a larger --bmaxdivn" << endl;
							throw BowtieException(err.str());
						}
