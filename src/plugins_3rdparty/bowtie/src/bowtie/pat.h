...
#define RESET_BUF_LEN(str, buf, length, typ) _setBegin(str, (typ*)buf); _setLength(str, length); _setCapacity(str, BUF_SIZE);
...
public:
	 ...
	 */
	virtual pair<uint64_t,uint64_t> readCnt() const {
		uint64_t rets = U64(0), retp = U64(0);
		for(size_t i = 0; i < srca_.size(); i++) {
			if(srcb_[i] == NULL) {
...
public:
	FastaContinuousPatternSource(
		...
		nameChars_(0), bufCur_(0), subReadCnt_(U64(0))
	{
		resetForNextFile();
