...
long numopts, outgrno_cons, col, setsz;
long maxgrp;               /* max. no. of groups in all trees found  */

boolean trout, firsttree, noroot, outgropt_cons, didreroot, prntsets,
    treeprint_cons, goteof, strict, mr=false, mre=false,
    ml=false; /* initialized all false for Treedist */
extern boolean progress;
pointarray nodep_cons;
pointarray treenode;
...
{ /* enter a set of species into the hash table */
  ...
  while (!done) {
    ...
    if (grouping[i - 1] && same) {  /* if it is there, increment timesseen */
      *timesseen[i - 1] += times;
      lengths[i - 1] = nodep_cons[r->index - 1]->v;
      done = true;
...
void dupname2(Phylip_Char *name, node *p, node *thisV)
{
  ...
  if (p->tip) {
    ...
    q = p;
    while (p->next != q) {
      dupname2(name, p->next->back, thisV);
      p = p->next;
    }
  }
}  /* dupname2 */
...
void reroot(node *outgroup, long *nextnode)
{
  ...
  if (i == 2) {
    ...
    nodep_cons[root->index-1] = root->next; /* make root->next the primary node */
...
void reorient(node* n) {
  ...
  if ( n->tip ) return;
  if ( nodep_cons[n->index - 1] != n )  {
    nodep_cons[n->index - 1] = n;
    if ( n->back )
      n->v = n->back->v;
  }
...
void reordertips()
{
  ...
  for (i = 0; i < spp-1; i++) {
    for (j = i + 1; j < spp; j++) {
      if (samename(nayme[i], nodep_cons[j]->nayme)) {
        /* switch the pointers in
         * nodep[] and set index accordingly for each node. */
        t = nodep_cons[i];
        
        nodep_cons[i] = nodep_cons[j];
        nodep_cons[i]->index = i+1;
        
        nodep_cons[j] = t;
        nodep_cons[j]->index = j+1;
        ...
        long total_trees, long tip_count, FILE *intree)
{
  ...
  while (!eoff(intree)) {          /* go till end of input tree file */
    ...
    nextnode = 0;
    haslengths = true;
    allocate_nodep(&nodep_cons, &intree, &spp);
    assert(spp == tip_count);
    treeread(intree, &root, treenode, &goteof, &firsttree, nodep_cons, 
    ...
    ntrees += trweight;
    if (noroot) {
      reroot(nodep_cons[outgrno_cons - 1], &nextnode);
      didreroot = outgropt_cons;
    }
    accumulate(root);
    gdispose(root);
    for (j = 0; j < 2*(1+spp); j++)
