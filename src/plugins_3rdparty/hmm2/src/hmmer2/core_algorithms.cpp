...
*/
float
P7SmallViterbi(unsigned char *dsq, int L, struct plan7_s *hmm, struct dpmatrix_s *mx, struct p7trace_s **ret_tr, int& progress)
{
    ...
    */
    sc = P7ParsingViterbi(dsq, L, hmm, &ctr, progress);
    ...
    */
    ndom = ctr->tlen/2 - 1;
    tarr = (struct p7trace_s **)MallocOrDie(sizeof(struct p7trace_s *) * ndom);
    tlen = totlen = 0;
    for (i = 0; i < ndom; i++)
    {
...
*/
float
P7ParsingViterbi(unsigned char *dsq, int L, struct plan7_s *hmm, struct p7trace_s **ret_tr, int& progress)
{
    ...
    */
    ...
    btr = (int*)MallocOrDie(sizeof(int) * (L+1));
    etr = (int*)MallocOrDie(sizeof(int) * (L+1));
...
P7WeeViterbi(unsigned char *dsq, int L, struct plan7_s *hmm, struct p7trace_s **ret_tr)
{
    ...
    /* Initialize.
    */
    kassign   = (int*) MallocOrDie (sizeof(int) * (L+1));
    tassign   = (char*)MallocOrDie (sizeof(char)* (L+1));
    endlist   = (int*) MallocOrDie (sizeof(int) * (L+1));
    startlist = (int*) MallocOrDie (sizeof(int) * (L+1));
    ...
    P7ViterbiAlignAlignment(MSA *msa, struct plan7_s *hmm)
{
    ...
    */
    /* allocation */
    con  = (float**)MallocOrDie(sizeof(float *) * (msa->alen+1));
    mocc = (float*)MallocOrDie(sizeof(float)   * (msa->alen+1));
    for (i = 1; i <= msa->alen; i++) {
        con[i] = (float*)MallocOrDie(sizeof(float) * al->Alphabet_size);
        FSet(con[i], al->Alphabet_size, 0.0);
    }
    mocc[0] = -9999.;
    /* initialization */
    ...
    for (i = 1; i <= msa->alen; i++)
    {
        for (idx = 0; idx < msa->nseq; idx++)
            if (! isgap(msa->aseq[idx][i-1]))
                P7CountSymbol(con[i], SYMIDX(al, msa->aseq[idx][i-1]), msa->wgt[idx]);
        FScale(con[i], al->Alphabet_size, 1./denom);
        mocc[i] = FSum(con[i], al->Alphabet_size);
    }
    ...
    */
    for (i = 1; i <= msa->alen; i++) {
        ...
        for (k = 1; k <= hmm->M; k++) {
            ...
            for (sym = 0; sym < al->Alphabet_size; sym++)
            {
...
*/
float
//PostprocessSignificantHit(struct alphabet_s * al, struct tophit_s    *ghit, 
    ...
    int                 hmmpfam_mode)
{
    ...
    */
    score     = (float*)MallocOrDie(sizeof(float) * ntr);
    usedomain = (int*)MallocOrDie(sizeof(int)   * ntr);
    ndom      = 0;
    whole_sc  = 0.;
