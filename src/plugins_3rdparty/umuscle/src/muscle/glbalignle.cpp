...
*/
typedef MuscleContext::glbalignle_struct::DP_MEMORY DP_MEMORY;

static void AllocDPMem(unsigned uLengthA, unsigned uLengthB)
	{
    DP_MEMORY &DPM = getMuscleContext()->glbalignle.DPM;
...
SCORE GlobalAlignLE(const ProfPos *PA, unsigned uLengthA, const ProfPos *PB,
  unsigned uLengthB, PWPath &Path)
    ...
    DP_MEMORY &DPM = ctx->glbalignle.DPM;
    SCORE &g_scoreCenter = ctx->params.g_scoreCenter;
