...
void MSA::GetFractionalWeightedCounts(unsigned uColIndex, bool bNormalize,
  ...
  FCOUNT *ptrfcLL, FCOUNT *ptrfcLG, FCOUNT *ptrfcGL, FCOUNT *ptrfcGG,
  MuscleContext* ctx) const
	...
	for (unsigned uSeqIndex = 0; uSeqIndex < uSeqCount; ++uSeqIndex)
		...
		else if (IsWildcard(uSeqIndex, uColIndex, g_IsWildcardChar))
			{
			const unsigned uLetter = GetLetterEx(uSeqIndex, uColIndex, g_CharToLetterEx);
			switch (g_Alpha)
				{
			case ALPHA_Amino:
				switch (uLetter)
		...
		unsigned uLetter = GetLetter(uSeqIndex, uColIndex, g_CharToLetter);
		fcCounts[uLetter] += w;
		wTotal += w;
		}
